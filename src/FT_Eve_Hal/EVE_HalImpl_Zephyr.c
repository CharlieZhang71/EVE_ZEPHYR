...
#if defined(ZEPHYR_PLATFORM)/* Initialize HAL platform */
...
int sss = 0;
#define EVE_CS_ACTIVE()		   {}
#define EVE_CS_DEACTIVE()      {}
//#define EVE_CS_ACTIVE()        { HAL_GPIO_WritePin(EVE_CS_PORT, EVE_CS_PIN, GPIO_PIN_RESET);}
//#define EVE_CS_DEACTIVE()      { HAL_GPIO_WritePin(EVE_CS_PORT, EVE_CS_PIN, GPIO_PIN_SET);}
/*#ifdef __GNUC__
...
void EVE_Hal_startTransfer(EVE_HalContext *phost, EVE_TRANSFER_T rw,
	...
	if (rw == EVE_TRANSFER_READ) {
		...
		//ret = HAL_SPI_Transmit(SpiHandle, spiRdHdr, sizeof(spiRdHdr), HAL_MAX_DELAY);
		...
		//ret = HAL_SPI_Transmit(SpiHandle, spiWrHdr, sizeof(spiWrHdr), HAL_MAX_DELAY);
