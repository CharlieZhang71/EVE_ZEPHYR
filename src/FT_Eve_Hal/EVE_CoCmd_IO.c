...
EVE_HAL_EXPORT bool EVE_CoCmd_getImage_format(EVE_HalContext *phost, uint32_t *format)
{
	...
	if (EVE_CHIPID >= EVE_BT817)
	{
		...
		resAddr = EVE_Cmd_moveWp(phost, 4 * 5); // move write pointer to result location
		EVE_Cmd_endFunc(phost);
		...
		if (format)
			*format = EVE_Hal_rd32(phost, RAM_CMD + resAddr + 4); // Read format from result
	}
	else
#endif
	{
...
EVE_HAL_EXPORT uint32_t EVE_CoCmd_fsDir_s(EVE_HalContext *phost, uint32_t dst, uint32_t num, const char *path, uint32_t length)
{
	...
	EVE_MULTI_TARGET_CHECK_RETURN(CMD_FSDIR, EVE_CHIPID >= EVE_BT820, 0xFFFFFFFF);

#if EVE_CMD_HOOKS
	if (phost->CoCmdHook && phost->CoCmdHook(phost, CMD_FSDIR, dst))
		return 0xFFFFFFFF;
#endif
...
EVE_HAL_EXPORT uint32_t EVE_CoCmd_fsDir_ex_s(EVE_HalContext *phost, uint8_t *dst, uint32_t tmp, uint32_t num, const char *path, uint32_t length)
{
	...
	/* If command succeeded, copy data from GPU RAM to system memory */
	if (result == 0)
	{
...
EVE_HAL_EXPORT uint32_t EVE_CoCmd_fsRead_s(EVE_HalContext *phost, uint32_t dst, const char *filename, uint32_t length)
{
	...
	EVE_MULTI_TARGET_CHECK_RETURN(CMD_FSREAD, EVE_CHIPID >= EVE_BT820, 0xFFFFFFFF);

#if EVE_CMD_HOOKS
	if (phost->CoCmdHook && phost->CoCmdHook(phost, CMD_FSREAD, dst))
		return 0xFFFFFFFF;
#endif
...
EVE_HAL_EXPORT uint32_t EVE_CoCmd_fsSize_s(EVE_HalContext *phost, const char *filename, uint32_t length)
{
	...
	EVE_MULTI_TARGET_CHECK_RETURN(CMD_FSSIZE, EVE_CHIPID >= EVE_BT820, 0xFFFFFFFF);
...
EVE_HAL_EXPORT uint32_t EVE_CoCmd_fsSource_s(EVE_HalContext *phost, const char *filename, uint32_t length)
{
	...
	EVE_MULTI_TARGET_CHECK_RETURN(CMD_FSSOURCE, EVE_CHIPID >= EVE_BT820, 0xFFFFFFFF);
...
EVE_HAL_EXPORT uint32_t EVE_CoCmd_sdAttach(EVE_HalContext *phost, uint32_t options)
{
	...
	EVE_MULTI_TARGET_CHECK_RETURN(CMD_SDATTACH, EVE_CHIPID >= EVE_BT820, 0xFFFFFFFF);

#if EVE_CMD_HOOKS
	if (phost->CoCmdHook && phost->CoCmdHook(phost, CMD_SDATTACH, options))
		return 0xFFFFFFFF;
#endif
...
EVE_HAL_EXPORT uint32_t EVE_CoCmd_sdBlockRead(EVE_HalContext *phost, uint32_t dst, uint32_t src, uint32_t count)
{
	...
	EVE_MULTI_TARGET_CHECK_RETURN(CMD_SDBLOCKREAD, EVE_CHIPID >= EVE_BT820, 0xFFFFFFFF);

#if EVE_CMD_HOOKS
	if (phost->CoCmdHook && phost->CoCmdHook(phost, CMD_SDBLOCKREAD, dst))
		return 0xFFFFFFFF;
#endif
...
EVE_HAL_EXPORT bool EVE_CoCmd_loadImage_sdCard(EVE_HalContext *phost, uint32_t address, char *filename, uint32_t *format)
{
	if (!EVE_Cmd_waitFlush(phost))
		return false; // Coprocessor must be ready
	EVE_Cmd_startFunc(phost);
...
EVE_HAL_EXPORT bool EVE_CoCmd_inflate_sdCard(EVE_HalContext *phost, uint32_t address, char *filename)
{
	if (!EVE_Cmd_waitFlush(phost))
		return false; // Coprocessor must be ready
	EVE_Cmd_startFunc(phost);
