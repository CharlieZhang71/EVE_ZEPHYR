...
EVE_HAL_EXPORT void EVE_CoCmd_dddddd(EVE_HalContext *phost, uint32_t cmd,
...
EVE_HAL_EXPORT void EVE_CoCmd_ddwwz(EVE_HalContext *phost, uint32_t cmd,
    uint32_t d0, uint16_t w1, uint16_t w2, const char *s);
EVE_HAL_EXPORT void EVE_CoCmd_ddwwz_s(EVE_HalContext *phost, uint32_t cmd,
...
EVE_HAL_EXPORT void EVE_CoCmd_dwwwwww(EVE_HalContext *phost, uint32_t cmd,
...
EVE_HAL_EXPORT void EVE_CoCmd_ddwwwwdd(EVE_HalContext *phost, uint32_t cmd,
...
#if (EVE_SUPPORT_CHIPID >= EVE_BT817) && (EVE_SUPPORT_CHIPID < EVE_BT820 || defined(EVE_MULTI_GRAPHICS_TARGET))
/**
 * @brief Send CMD_HSF
 *
 * @param phost Pointer to Hal context
...
ESD_FUNCTION(EVE_CoCmd_endList, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
...
{
	EVE_MULTI_TARGET_CHECK(CMD_ENDLIST, EVE_CHIPID >= EVE_BT820);
...
ESD_FUNCTION(EVE_CoCmd_copyList, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) 
ESD_PARAMETER(dst, Type = uint32_t, Default = 0) 
/*
 * @brief Send CMD_COPYLIST to copy current display list into RAM
 *
 * @param phost Pointer to Hal context
 * @param dst Destination address for the display list, 4-byte aligned.
 *           If -1 is given as the address, then the current allocation pointer is used instead.
 */
static inline void EVE_CoCmd_copyList(EVE_HalContext *phost, uint32_t dst)
{
	EVE_MULTI_TARGET_CHECK(CMD_COPYLIST, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_dd(phost, CMD_COPYLIST, dst);
}

ESD_FUNCTION(EVE_CoCmd_renderTarget, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) 
ESD_PARAMETER(dst, Type = uint32_t, Default = 0) 
ESD_PARAMETER(fmt, Type = uint16_t, Default = L8)
ESD_PARAMETER(w, Type = int16_t, Default = 0) // Must be multiple of 16
ESD_PARAMETER(h, Type = int16_t, Default = 0) // w × h must be multiple of 128
/*
 * @brief Set the render target registers
 *
 * @param phost Pointer to Hal context
 * @param dst Render target start address, either SWAPCHAIN_0 or a 128-byte aligned memory address
 * @param fmt Render target format: L8, LA8, RGB565, RGB6, RGB8, ARGB4, ARGB1555, ARGB6, ARGB8, YCBCR
 * @param w Render target width in pixels, must be a multiple of 16
 * @param h Render target height in pixels. w × h must be a multiple of 128
 */
static inline void EVE_CoCmd_renderTarget(EVE_HalContext *phost, uint32_t dst,
    uint16_t fmt, int16_t w, int16_t h)
{
	EVE_MULTI_TARGET_CHECK(CMD_RENDERTARGET, EVE_CHIPID >= EVE_BT820);
	if (EVE_CMD_CACHE_RENDER_TARGET)
	{
		/* Cache the render target for restoring after indermediate display list construction */
...
#define EVE_CoCmd_copyList(phost, dst) EVE_COCMD_UNSUPPORTED(CMD_COPYLIST, false)
#define EVE_CoCmd_renderTarget(phost, dst, fmt, w, h) EVE_COCMD_UNSUPPORTED(CMD_RENDERTARGET, false)
...
ESD_FUNCTION(EVE_CoCmd_apbRead, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(dest, Type = uint32_t, Default = 0) // MEMORY_ADDRESS
ESD_PARAMETER(addr, Type = uint32_t, Default = 0)
/**
 * @brief Send CMD_APBREAD to read from APB register
 *
 * @param phost Pointer to Hal context
 * @param dest Main memory destination location
 * @param addr APB address
 */
static inline void EVE_CoCmd_apbRead(EVE_HalContext *phost, uint32_t dest, uint32_t addr)
{
	EVE_MULTI_TARGET_CHECK(CMD_APBREAD, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_ddd(phost, CMD_APBREAD, dest, addr);
}

ESD_FUNCTION(EVE_CoCmd_apbWrite, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(addr, Type = uint32_t, Default = 0)
ESD_PARAMETER(v, Type = uint32_t, Default = 0)
/**
 * @brief Send CMD_APBWRITE to write to APB register
 *
 * @param phost Pointer to Hal context
 * @param addr APB address
 * @param v Value to write
 */
static inline void EVE_CoCmd_apbWrite(EVE_HalContext *phost, uint32_t addr, uint32_t v)
{
	EVE_MULTI_TARGET_CHECK(CMD_APBWRITE, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_ddd(phost, CMD_APBWRITE, addr, v);
}

ESD_FUNCTION(EVE_CoCmd_regWrite, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(dst, Type = uint32_t, Default = 0) // REGISTER_ADDRESS
ESD_PARAMETER(value, Type = uint32_t, Default = 0)
/**
 * @brief Send CMD_REGWRITE to write a value to a system register
 *
 * @param phost Pointer to Hal context
 * @param dst Register address to write, must be 32-bit aligned
 * @param value 32-bit value to write
 */
static inline void EVE_CoCmd_regWrite(EVE_HalContext *phost, uint32_t dst, uint32_t value)
{
	EVE_MULTI_TARGET_CHECK(CMD_REGWRITE, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_ddd(phost, CMD_REGWRITE, dst, value);
}

ESD_FUNCTION(EVE_CoCmd_result, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(dst, Type = uint32_t, Default = 0) // MEMORY_ADDRESS
/**
 * @brief Send CMD_RESULT to write the result of the preceding command into memory
 *
 * @param phost Pointer to Hal context
 * @param dst Memory address to write, must be 32-bit aligned
 */
static inline void EVE_CoCmd_result(EVE_HalContext *phost, uint32_t dst)
{
	EVE_MULTI_TARGET_CHECK(CMD_RESULT, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_dd(phost, CMD_RESULT, dst);
}

ESD_FUNCTION(EVE_CoCmd_waitChange, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(a, Type = uint32_t, Default = 0) // RAM_REG
/**
 * @brief Send CMD_WAITCHANGE to wait for a register value to change
 *
 * @param phost Pointer to Hal context
 * @param a Address of RAM or register to wait on, 32-bit aligned
 */
static inline void EVE_CoCmd_waitChange(EVE_HalContext *phost, uint32_t a)
{
	EVE_MULTI_TARGET_CHECK(CMD_WAITCHANGE, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_dd(phost, CMD_WAITCHANGE, a);
}

ESD_FUNCTION(EVE_CoCmd_waitCond, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(a, Type = uint32_t, Default = 0) // RAM_REG
ESD_PARAMETER(func, Type = uint32_t, Default = NEVER)
ESD_PARAMETER(ref, Type = uint32_t, Default = 0)
ESD_PARAMETER(mask, Type = uint32_t, Default = 0xFFFFFFFF)
/**
 * @brief Send CMD_WAITCOND to wait until a given condition is true
 *
 * @param phost Pointer to Hal context
 * @param a Address of RAM or register to check, 32-bit aligned
 * @param func Comparison function: NEVER, LESS, LEQUAL, GREATER, GEQUAL, EQUAL, NOTEQUAL, or ALWAYS
 * @param ref 32-bit reference value
 * @param mask 32-bit mask
 *
 * @note The condition is checked as: (*a & mask) <func> (ref & mask)
 * @note ALWAYS never waits and NEVER waits indefinitely
 */
static inline void EVE_CoCmd_waitCond(EVE_HalContext *phost, uint32_t a,
    uint32_t func, uint32_t ref, uint32_t mask)
{
	EVE_MULTI_TARGET_CHECK(CMD_WAITCOND, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_ddddd(phost, CMD_WAITCOND, a, func, ref, mask);
}

ESD_FUNCTION(EVE_CoCmd_fence, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
/**
 * @brief Send CMD_FENCE - wait for outstanding writes to complete
 *
 * @param phost Pointer to Hal context
 */
static inline void EVE_CoCmd_fence(EVE_HalContext *phost)
{
	EVE_MULTI_TARGET_CHECK(CMD_FENCE, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_d(phost, CMD_FENCE);
}

ESD_FUNCTION(EVE_CoCmd_workArea, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(ptr, Type = uint32_t, Default = 0) // MEMORY_ADDRESS
/**
 * @brief Send CMD_WORKAREA to set the coprocessor work area pointer
 *
 * @param phost Pointer to Hal context
 * @param ptr Pointer to the work area, must be 4K byte aligned
 *
 * @note Sets the 1.5 MB coprocessor work area to a location in main memory
 */
static inline void EVE_CoCmd_workArea(EVE_HalContext *phost, uint32_t ptr)
{
	EVE_MULTI_TARGET_CHECK(CMD_WORKAREA, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_dd(phost, CMD_WORKAREA, ptr);
}

ESD_FUNCTION(EVE_CoCmd_skipCond, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(a, Type = uint32_t, Default = 0) // MEMORY_ADDRESS
ESD_PARAMETER(func, Type = uint32_t, Default = 0)
ESD_PARAMETER(ref, Type = uint32_t, Default = 0)
ESD_PARAMETER(mask, Type = uint32_t, Default = 0)
ESD_PARAMETER(num, Type = uint32_t, Default = 0)
/**
 * @brief Send CMD_SKIPCOND to skip following command bytes if condition is true
 *
 * @param phost Pointer to Hal context
 * @param a Address of RAM or register to check, 32-bit aligned
 * @param func Function to compare: NEVER, LESS, LEQUAL, GREATER, GEQUAL, EQUAL, NOTEQUAL, or ALWAYS
 * @param ref 32-bit reference value
 * @param mask 32-bit mask
 * @param num Number of bytes to skip if condition is true
 */
static inline void EVE_CoCmd_skipCond(EVE_HalContext *phost, uint32_t a,
    uint32_t func, uint32_t ref, uint32_t mask, uint32_t num)
{
	EVE_MULTI_TARGET_CHECK(CMD_SKIPCOND, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_dddddd(phost, CMD_SKIPCOND, a, func, ref, mask, num);
}

#else

#define EVE_CoCmd_apbRead(phost, dest, addr) EVE_COCMD_UNSUPPORTED(CMD_APBREAD, false)
#define EVE_CoCmd_apbWrite(phost, addr, v) EVE_COCMD_UNSUPPORTED(CMD_APBWRITE, false)
#define EVE_CoCmd_regWrite(phost, dst, value) EVE_COCMD_UNSUPPORTED(CMD_REGWRITE, false)
#define EVE_CoCmd_result(phost, dst) EVE_COCMD_UNSUPPORTED(CMD_RESULT, false)
#define EVE_CoCmd_waitChange(phost, a) EVE_COCMD_UNSUPPORTED(CMD_WAITCHANGE, false)
#define EVE_CoCmd_waitCond(phost, a, func, ref, mask) EVE_COCMD_UNSUPPORTED(CMD_WAITCOND, false)
#define EVE_CoCmd_workArea(phost, ptr) EVE_COCMD_UNSUPPORTED(CMD_WORKAREA, false)
#define EVE_CoCmd_fence(phost) EVE_COCMD_UNSUPPORTED(CMD_FENCE, false)
#define EVE_CoCmd_skipCond(phost, a, func, ref, mask, num) EVE_COCMD_UNSUPPORTED(CMD_SKIPCOND, false)
...
#if (EVE_SUPPORT_CHIPID >= EVE_BT820)
	if (EVE_CHIPID == EVE_BT820 && ((options & OPT_FS) || (options & OPT_FLASH)))
	{
		EVE_Cmd_wr32(phost, CMD_NOP); // WORKAROUND: Early return of CMD_LOADIMAGE when no data follows
	}
#endif
}

// CMD_GETIMAGE but only reading out the format, with backwards compatibility using hardcoded 0x3097e8 address
EVE_HAL_EXPORT bool EVE_CoCmd_getImage_format(EVE_HalContext *phost, uint32_t *format);
...
ESD_FUNCTION(EVE_CoCmd_textDim, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(dimensions, Type = uint32_t, Default = 0) // MEMORY_ADDRESS
ESD_PARAMETER(font, Type = int16_t, Default = 21, Min = 0, Max = 63) // BITMAP_HANDLE
ESD_PARAMETER(options, Type = Ft_CoPro_Opt, Default = 0)
ESD_PARAMETER(s, Type = const char *, Default = "Text")
/**
 * @brief Send CMD_TEXTDIM to compute the size of a UTF-8 text string
 *
 * @param phost Pointer to Hal context
 * @param dimensions Pointer to 8-byte result area (4-byte aligned) for width and height in pixels
 * @param font Font to use for text, 0-63
 * @param options OPT_FORMAT for format string, OPT_FILL for multi-line text with CMD_FILLWIDTH
 * @param s Text string, UTF-8 encoding. Can contain newlines if OPT_FILL not used
 */
static inline void EVE_CoCmd_textDim(EVE_HalContext *phost, uint32_t dimensions,
    int16_t font, uint16_t options, const char *s)
{
	EVE_MULTI_TARGET_CHECK(CMD_TEXTDIM, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_ddwwz(phost, CMD_TEXTDIM, dimensions, font, options, s);
}

static inline void EVE_CoCmd_textDim_s(EVE_HalContext *phost, uint32_t dimensions,
...
#define EVE_CoCmd_textDim(phost, dimensions, font, options, s) EVE_COCMD_UNSUPPORTED(CMD_TEXTDIM, false)
#define EVE_CoCmd_textDim_s(phost, dimensions, font, options, s, len) EVE_COCMD_UNSUPPORTED(CMD_TEXTDIM, false)
...
{
	EVE_MULTI_TARGET_CHECK(CMD_MEDIAFIFO, EVE_CHIPID >= EVE_FT810 && EVE_CHIPID < EVE_BT820);
...
ESD_FUNCTION(EVE_CoCmd_videoStart_ex, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(options, Type = uint32_t, Default = 0)
/**
* @brief Send CMD_VIDEOSTART to initialize video frame decoder with options
* 
* @param phost Pointer to Hal context
* @param options Video options:
*      Source (required): OPT_FLASH, OPT_FS, or OPT_MEDIAFIFO
*      Format (optional): OPT_YCBCR or OPT_TRUECOLOR (default is RGB565)
*/
static inline void EVE_CoCmd_videoStart_ex(EVE_HalContext *phost, uint32_t options)
{
	EVE_MULTI_TARGET_CHECK(CMD_VIDEOSTART, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_dd(phost, CMD_VIDEOSTART, options);
}

#else

#define EVE_CoCmd_videoStart_ex(phost, options) EVE_COCMD_UNSUPPORTED(CMD_VIDEOSTART, false)
...
/**
* @brief Send CMD_VIDEOSTARTF to initialize video frame decoder for flash video
* The video data must be present in flash memory, with address set using CMD_FLASHSOURCE
* 
* @param phost Pointer to Hal context
*/
...
ESD_FUNCTION(EVE_CoCmd_saveContext, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
/**
 * @brief Save current graphics state to the state stack
 *
 * @param phost Pointer to Hal context
 */
static inline void EVE_CoCmd_saveContext(EVE_HalContext *phost)
{
...
ESD_FUNCTION(EVE_CoCmd_restoreContext, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
/**
 * @brief Restore graphics state from the stack
 *
 * @param phost Pointer to Hal context
 */
static inline void EVE_CoCmd_restoreContext(EVE_HalContext *phost)
{
#if (EVE_SUPPORT_CHIPID >= EVE_BT820)
	if (EVE_CHIPID >= EVE_BT820)
	{
...
ESD_FUNCTION(EVE_CoCmd_cGradient, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(shape, Type = uint32_t, Default = 0)
ESD_PARAMETER(x, Type = int16_t, Default = 0) // SCREEN_SIZE
ESD_PARAMETER(y, Type = int16_t, Default = 0) // SCREEN_SIZE
ESD_PARAMETER(w, Type = int16_t, Default = 0) // SCREEN_SIZE
ESD_PARAMETER(h, Type = int16_t, Default = 0) // SCREEN_SIZE
ESD_PARAMETER(rgb0, Type = esd_rgb32_t, Default = 0) // COLOR
ESD_PARAMETER(rgb1, Type = esd_rgb32_t, Default = 0) // COLOR
/**
 * @brief Send CMD_CGRADIENT to draw a circular gradient
 *
 * @param phost Pointer to Hal context
 * @param shape Gradient shape (CORNER_ZERO or EDGE_ZERO)
 ...
 * @param rgb0 Outer color, as a 24-bit RGB number
 * @param rgb1 Inner color, as a 24-bit RGB number
 */
static inline void EVE_CoCmd_cGradient(EVE_HalContext *phost, uint32_t shape,
    int16_t x, int16_t y, int16_t w, int16_t h,
    uint32_t rgb0, uint32_t rgb1)
{
	EVE_MULTI_TARGET_CHECK(CMD_CGRADIENT, EVE_CHIPID >= EVE_BT820);
...
ESD_FUNCTION(EVE_CoCmd_glow, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(x, Type = int16_t, Default = 0) // SCREEN_SIZE
ESD_PARAMETER(y, Type = int16_t, Default = 0) // SCREEN_SIZE
ESD_PARAMETER(w, Type = int16_t, Default = 0) // SCREEN_SIZE
ESD_PARAMETER(h, Type = int16_t, Default = 0) // SCREEN_SIZE
/**
 * @brief Send CMD_GLOW to draw an additive glow effect
 *
 * @param phost Pointer to Hal context
 * @param x x-coordinate of top-left of glow rectangle, in pixels
 * @param y y-coordinate of top-left of glow rectangle, in pixels
 * @param w width of rectangle in pixels
 * @param h height of rectangle in pixels
 */
static inline void EVE_CoCmd_glow(EVE_HalContext *phost,
    int16_t x, int16_t y, int16_t w, int16_t h)
{
	EVE_MULTI_TARGET_CHECK(CMD_GLOW, EVE_CHIPID >= EVE_BT820);
...
ESD_FUNCTION(EVE_CoCmd_arc, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(x, Type = int16_t, Default = 0) // SCREEN_SIZE
ESD_PARAMETER(y, Type = int16_t, Default = 0) // SCREEN_SIZE
ESD_PARAMETER(r0, Type = uint16_t, Default = 0, Min = 1, Max = 511)
ESD_PARAMETER(r1, Type = uint16_t, Default = 0, Min = 1, Max = 511)
ESD_PARAMETER(a0, Type = uint16_t, Default = 0)
ESD_PARAMETER(a1, Type = uint16_t, Default = 0)
/**
* @brief Send CMD_ARC to draw a circular arc
* 
* @param phost Pointer to Hal context
* @param x x-coordinate of arc center, in pixels
* @param y y-coordinate of arc center, in pixels
* @param r0 inner radius, in pixels, 1-511
* @param r1 outer radius, in pixels, 1-511 
* @param a0 arc starting angle, in furmans (0x0000-0xffff)
* @param a1 arc ending angle, in furmans (0x0000-0xffff)
*/
static inline void EVE_CoCmd_arc(EVE_HalContext *phost, int16_t x, int16_t y, uint16_t r0, uint16_t r1, uint16_t a0, uint16_t a1)
{
	EVE_MULTI_TARGET_CHECK(CMD_ARC, EVE_CHIPID >= EVE_BT820);
...
#define EVE_CoCmd_cGradient(phost, shape, x, y, w, h, rgb0, rgb1) EVE_COCMD_UNSUPPORTED(CMD_CGRADIENT, false)
#define EVE_CoCmd_glow(phost, x, y, w, h) EVE_COCMD_UNSUPPORTED(CMD_GLOW, false)
#define EVE_CoCmd_arc(phost, x, y, r0, r1, a0, a1) EVE_COCMD_UNSUPPORTED(CMD_ARC, false)
...
#if (EVE_SUPPORT_CHIPID < EVE_BT820 || defined(EVE_MULTI_GRAPHICS_TARGET)) // TODO: VERIFY IF THIS WORKAROUND IS REALLY NO LONGER NEEDED ON BT820
	if (EVE_CHIPID < EVE_BT820 && (options & (OPT_FLASH | OPT_MEDIAFIFO)))
	{
...
ESD_FUNCTION(EVE_CoCmd_enableRegion, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(en, Type = uint32_t, Default = 0)
/**
 * @brief Send CMD_ENABLEREGION to enable/disable REGION optimization
 *
 * @param phost Pointer to Hal context
 * @param en Enable flag for all REGION optimizations
 */
static inline void EVE_CoCmd_enableRegion(EVE_HalContext *phost, uint32_t en)
{
	EVE_MULTI_TARGET_CHECK(CMD_ENABLEREGION, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_dd(phost, CMD_ENABLEREGION, en);
}

#if 0 // FIXME: MISSING DEFINITION OF CMD_REGION
ESD_FUNCTION(EVE_CoCmd_region, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
/**
* @brief Send CMD_REGION to indicate start of a region section
* 
* @param phost Pointer to Hal context
*/
static inline void EVE_CoCmd_region(EVE_HalContext *phost)
{
	EVE_MULTI_TARGET_CHECK(CMD_REGION, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_d(phost, CMD_REGION);
}
#else
#define EVE_CoCmd_region(phost) EVE_COCMD_UNSUPPORTED(CMD_REGION, false)
#endif

#if 0 // FIXME: MISSING DEFINITION OF CMD_ENDREGION
ESD_FUNCTION(EVE_CoCmd_endRegion, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(x, Type = int16_t, Default = 0) // SCREEN_SIZE
ESD_PARAMETER(y, Type = int16_t, Default = 0) // SCREEN_SIZE
ESD_PARAMETER(w, Type = int16_t, Default = 0) // SCREEN_SIZE
ESD_PARAMETER(h, Type = int16_t, Default = 0) // SCREEN_SIZE
/**
* @brief Send CMD_ENDREGION to mark the end of a region section
* 
* @param phost Pointer to Hal context
* @param x x-coordinate of top-left of region rectangle, in pixels
* @param y y-coordinate of top-left of region rectangle, in pixels
* @param w width of rectangle in pixels 
* @param h height of rectangle in pixels
*/
static inline void EVE_CoCmd_endRegion(EVE_HalContext *phost,
	int16_t x, int16_t y, int16_t w, int16_t h)
{
	EVE_MULTI_TARGET_CHECK(CMD_ENDREGION, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_dwwww(phost, CMD_ENDREGION, x, y, w, h);
}
#else
...
#define EVE_CoCmd_enableRegion(phost, en) EVE_COCMD_UNSUPPORTED(CMD_ENABLEREGION, false)
#define EVE_CoCmd_region(phost) EVE_COCMD_UNSUPPORTED(CMD_REGION, false)
#define EVE_CoCmd_endRegion(phost, x, y, w, h) EVE_COCMD_UNSUPPORTED(CMD_ENDREGION, false)
...
ESD_FUNCTION(EVE_CoCmd_fsDir, Type = void, Category = _GroupHidden, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(dst, Type = uint32_t, Default = 0) // RAM_G
ESD_PARAMETER(num, Type = uint32_t, Default = 0)
ESD_PARAMETER(path, Type = const char *, Default = "/")
/**
 * @brief Send CMD_FSDIR to list files on SDcard
 *
 * @param phost Pointer to Hal context
 * @param dst Start of buffer
 * @param num Size of buffer, in bytes
 * @param path Path of directory to list. Must end with directory separator
 * @return uint32_t Result code: 0 on success, 2 (ENOENT) file not found, 5 (EIO) IO error, 12 (ENOMEM) buffer too small, or 0xFFFFFFFF on coprocessor fault
 */
EVE_HAL_EXPORT uint32_t EVE_CoCmd_fsDir(EVE_HalContext *phost, uint32_t dst, uint32_t num, const char *path);

/**
 * @brief Send CMD_FSDIR with string length to list files on SDcard
 *
 * @param phost Pointer to Hal context
 * @param dst Start of buffer
 * @param num Size of buffer, in bytes
 * @param path Path of directory to list. Must end with directory separator
 * @param length Length of path string
 * @return uint32_t Result code: 0 on success, 2 (ENOENT) file not found, 5 (EIO) IO error, 12 (ENOMEM) buffer too small, or 0xFFFFFFFF on coprocessor fault
 */
EVE_HAL_EXPORT uint32_t EVE_CoCmd_fsDir_s(EVE_HalContext *phost, uint32_t dst, uint32_t num, const char *path, uint32_t length);

/**
* @brief Send CMD_FSDIR to list files on SDcard and copy result to system memory
*
* @param phost Pointer to Hal context
* @param dst Pointer to system memory buffer to store directory listing
* @param tmp Temporary GPU RAM address to use for transfer
* @param num Size of buffer, in bytes
* @param path Path of directory to list. Must end with directory separator
* @return uint32_t Result code: 0 on success, 2 (ENOENT) file not found, 5 (EIO) IO error, 12 (ENOMEM) buffer too small, or 0xFFFFFFFF on coprocessor fault
*/
EVE_HAL_EXPORT uint32_t EVE_CoCmd_fsDir_ex(EVE_HalContext *phost, uint8_t *dst, uint32_t tmp, uint32_t num, const char *path);

/**
* @brief Send CMD_FSDIR with string length to list files on SDcard and copy result to system memory
*
* @param phost Pointer to Hal context
* @param dst Pointer to system memory buffer to store directory listing
* @param tmp Temporary GPU RAM address to use for transfer
...
* @return uint32_t Result code: 0 on success, 2 (ENOENT) file not found, 5 (EIO) IO error, 12 (ENOMEM) buffer too small, or 0xFFFFFFFF on coprocessor fault
*/
EVE_HAL_EXPORT uint32_t EVE_CoCmd_fsDir_ex_s(EVE_HalContext *phost, uint8_t *dst, uint32_t tmp, uint32_t num, const char *path, uint32_t length);

ESD_FUNCTION(EVE_CoCmd_fsOptions, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(options, Type = uint32_t, Default = OPT_SFNLOWER | OPT_DIRSEP_WIN | OPT_DIRSEP_UNIX)
/**
 * @brief Send CMD_FSOPTIONS to set FAT subsystem options
 *
 * @param phost Pointer to Hal context
 * @param options Option bitfield. Combination of:
 *               OPT_SFNLOWER - FAT32 short filenames mapped to lowercase
 *               OPT_CASESENSITIVE - Filename comparison is case sensitive
 *               OPT_DIRSEP_WIN - Backslash as directory separator
 *               OPT_DIRSEP_UNIX - Forward slash as directory separator
 */
static inline void EVE_CoCmd_fsOptions(EVE_HalContext *phost, uint32_t options)
{
	EVE_MULTI_TARGET_CHECK(CMD_FSOPTIONS, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_dd(phost, CMD_FSOPTIONS, options);
}

ESD_FUNCTION(EVE_CoCmd_fsRead, Type = void, Category = _GroupHidden, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(dst, Type = uint32_t, Default = 0) // RAM_G
ESD_PARAMETER(filename, Type = const char *, Default = "")
/**
 * @brief Send CMD_FSREAD to read file from filesystem to RAM
 *
 * @param phost Pointer to Hal context
 * @param dst Destination address in RAM, 32 byte aligned
 * @param filename Complete filename of the file
 * @return uint32_t Result code: 0 on success, 2 (ENOENT) file not found, 5 (EIO) IO error, or 0xFFFFFFFF on coprocessor fault
 */
EVE_HAL_EXPORT uint32_t EVE_CoCmd_fsRead(EVE_HalContext *phost, uint32_t dst, const char *filename);

/**
 * @brief Send CMD_FSREAD with string length to read file from filesystem to RAM
 *
 * @param phost Pointer to Hal context
 * @param dst Destination address in RAM, 32 byte aligned
 * @param filename Complete filename of the file
 ...
 * @return uint32_t Result code: 0 on success, 2 (ENOENT) file not found, 5 (EIO) IO error, or 0xFFFFFFFF on coprocessor fault
 */
EVE_HAL_EXPORT uint32_t EVE_CoCmd_fsRead_s(EVE_HalContext *phost, uint32_t dst, const char *filename, uint32_t length);

ESD_FUNCTION(EVE_CoCmd_fsSize, Type = void, Category = _GroupHidden, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(filename, Type = const char *, Default = "")
/**
 * @brief Send CMD_FSSIZE to find size of file
 *
 * @param phost Pointer to Hal context
 * @param filename Complete pathname of the file
 * @return uint32_t File size in bytes, or 0xFFFFFFFF if file not found or on coprocessor fault
 */
EVE_HAL_EXPORT uint32_t EVE_CoCmd_fsSize(EVE_HalContext *phost, const char *filename);

/**
 * @brief Send CMD_FSSIZE with string length to find size of file
 *
 * @param phost Pointer to Hal context
 * @param filename Complete pathname of the file
 * @param length Length of filename string
 * @return uint32_t File size in bytes, or 0xFFFFFFFF if file not found or on coprocessor fault
 */
EVE_HAL_EXPORT uint32_t EVE_CoCmd_fsSize_s(EVE_HalContext *phost, const char *filename, uint32_t length);

ESD_FUNCTION(EVE_CoCmd_fsSource, Type = void, Category = _GroupHidden, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(filename, Type = const char *, Default = "")
/**
 * @brief Send CMD_FSSOURCE to set source file for future load
 *
 * @param phost Pointer to Hal context
 * @param filename Complete filename of the file, UTF-8 encoded
 * @return uint32_t Result code: 0 on success, 2 (ENOENT) file not found, 5 (EIO) IO error, or 0xFFFFFFFF on coprocessor fault
 */
EVE_HAL_EXPORT uint32_t EVE_CoCmd_fsSource(EVE_HalContext *phost, const char *filename);

/**
 * @brief Send CMD_FSSOURCE with string length to set source file for future load
 *
 * @param phost Pointer to Hal context
 * @param filename Complete filename of the file, UTF-8 encoded
 * @param length Length of filename string
 * @return uint32_t Result code: 0 on success, 2 (ENOENT) file not found, 5 (EIO) IO error, or 0xFFFFFFFF on coprocessor fault
 */
EVE_HAL_EXPORT uint32_t EVE_CoCmd_fsSource_s(EVE_HalContext *phost, const char *filename, uint32_t length);

ESD_FUNCTION(EVE_CoCmd_sdAttach, Type = void, Category = _GroupHidden, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(options, Type = uint32_t, Default = 0)
/**
 * @brief Send CMD_SDATTACH to connect to SD storage
 *
 * @param phost Pointer to Hal context
 * @param options Flags: OPT_4BIT, OPT_HALFSPEED, OPT_QUARTERSPEED, OPT_IS_SD, OPT_IS_MMC
 * @return uint32_t Result code: 0 on success, or error code:
 *         0xd000: SD card not detected
 *         0xd001: No response from card
 *         0xd002: Card timeout during initialization
 *         0xd003: Sector 0 (MBR) not found
 *         0xd004: FAT filesystem not found
 *         0xd005: Card failed to enter high-speed mode
 *         0xFFFFFFFF: Coprocessor fault
 */
EVE_HAL_EXPORT uint32_t EVE_CoCmd_sdAttach(EVE_HalContext *phost, uint32_t options);

ESD_FUNCTION(EVE_CoCmd_sdBlockRead, Type = void, Category = _GroupHidden, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(dst, Type = uint32_t, Default = 0) // RAM_G
ESD_PARAMETER(src, Type = uint32_t, Default = 0)
ESD_PARAMETER(count, Type = uint32_t, Default = 1)
/**
 * @brief Send CMD_SDBLOCKREAD to read blocks from SD storage
 *
 * @param phost Pointer to Hal context
 * @param dst Destination in main memory, must be 4-byte aligned
 * @param src Source block address
 * @param count Number of 512-byte blocks to read
 * @return uint32_t Result code: 0 on success, or 0xFFFFFFFF on coprocessor fault
 */
EVE_HAL_EXPORT uint32_t EVE_CoCmd_sdBlockRead(EVE_HalContext *phost, uint32_t dst, uint32_t src, uint32_t count);

#else

#define EVE_CoCmd_fsDir(phost, dst, num, path, result) EVE_COCMD_UNSUPPORTED(CMD_FSDIR, 0xFFFFFFFF)
#define EVE_CoCmd_fsDir_s(phost, dst, num, path, length, result) EVE_COCMD_UNSUPPORTED(CMD_FSDIR, 0xFFFFFFFF)
#define EVE_CoCmd_fsOptions(phost, options) EVE_COCMD_UNSUPPORTED(CMD_FSOPTIONS, false)
#define EVE_CoCmd_fsRead(phost, dst, filename) EVE_COCMD_UNSUPPORTED(CMD_FSREAD, 0xFFFFFFFF)
#define EVE_CoCmd_fsRead_s(phost, dst, filename, length) EVE_COCMD_UNSUPPORTED(CMD_FSREAD, 0xFFFFFFFF)
#define EVE_CoCmd_fsSize(phost, filename) EVE_COCMD_UNSUPPORTED(CMD_FSSIZE, 0xFFFFFFFF)
#define EVE_CoCmd_fsSize_s(phost, filename, length) EVE_COCMD_UNSUPPORTED(CMD_FSSIZE, 0xFFFFFFFF)
#define EVE_CoCmd_fsSource(phost, filename) EVE_COCMD_UNSUPPORTED(CMD_FSSOURCE, 0xFFFFFFFF)
#define EVE_CoCmd_fsSource_s(phost, filename, length) EVE_COCMD_UNSUPPORTED(CMD_FSSOURCE, 0xFFFFFFFF)
#define EVE_CoCmd_sdAttach(phost, options) EVE_COCMD_UNSUPPORTED(CMD_SDATTACH, 0xFFFFFFFF)
#define EVE_CoCmd_sdBlockRead(phost, dst, src, count) EVE_COCMD_UNSUPPORTED(CMD_SDBLOCKREAD, 0xFFFFFFFF)
...
ESD_FUNCTION(EVE_CoCmd_loadAsset, Type = void, Category = _GroupHidden, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(ptr, Type = uint32_t, Default = 0) // RAM_G
ESD_PARAMETER(options, Type = uint32_t, Default = 0)
/**
 * @brief Send CMD_LOADASSET to load a relocatable asset
 *
 * @param phost Pointer to Hal context
 * @param ptr Destination address, or -1 to use current allocation pointer
 * @param options Source options: 0 (command buffer), OPT_FLASH, OPT_FS, or OPT_MEDIAFIFO
 */
static inline void EVE_CoCmd_loadAsset(EVE_HalContext *phost, uint32_t ptr, uint32_t options)
{
	EVE_MULTI_TARGET_CHECK(CMD_LOADASSET, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_ddd(phost, CMD_LOADASSET, ptr, options);
}

ESD_FUNCTION(EVE_CoCmd_loadPatch, Type = void, Category = _GroupHidden, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(options, Type = uint32_t, Default = 0)
/**
 * @brief Send CMD_LOADPATCH to load a firmware patch
 *
 * @param phost Pointer to Hal context
 * @param options Source options:
 *                - 0: Data follows in command buffer
 *                - OPT_FLASH: Source from flash memory
 *                - OPT_FS: Source from filesystem
 *                - OPT_MEDIAFIFO: Source from media FIFO
 */
static inline void EVE_CoCmd_loadPatch(EVE_HalContext *phost, uint32_t options)
{
	EVE_MULTI_TARGET_CHECK(CMD_LOADPATCH, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_dd(phost, CMD_LOADPATCH, options);
}

ESD_FUNCTION(EVE_CoCmd_loadWav, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(dst, Type = uint32_t, Default = 0) // RAM_G
ESD_PARAMETER(options, Type = uint32_t, Default = 0)
/**
 * @brief Send CMD_LOADWAV to load a WAV file for background playback
 *
 * @param phost Pointer to Hal context
 * @param dst Destination address in RAM_G, or -1 to use current allocation pointer
 * @param options Source options:
 *                OPT_FLASH: source from flash memory
 *                OPT_FS: source from filesystem
 *                OPT_MEDIAFIFO: source from media FIFO
 *                Default (0): source from command buffer
 */
static inline void EVE_CoCmd_loadWav(EVE_HalContext *phost, uint32_t dst, uint32_t options)
{
	EVE_MULTI_TARGET_CHECK(CMD_LOADWAV, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_ddd(phost, CMD_LOADWAV, dst, options);
}

ESD_FUNCTION(EVE_CoCmd_playWav, Type = void, Category = _GroupHidden, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(options, Type = uint32_t, Default = 0)
/**
 * @brief Send CMD_PLAYWAV to play a sound sample
 *
 * @param phost Pointer to Hal context
 * @param options Data source options:
 *                - 0: Data follows in command buffer
 *                - OPT_FLASH: Data from flash memory
 *                - OPT_FS: Data from filesystem
 *                - OPT_MEDIAFIFO: Data from media FIFO
 */
static inline void EVE_CoCmd_playWav(EVE_HalContext *phost, uint32_t options)
{
	EVE_MULTI_TARGET_CHECK(CMD_PLAYWAV, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_dd(phost, CMD_PLAYWAV, options);
}

#else

#define EVE_CoCmd_loadAsset(phost, ptr, options) EVE_COCMD_UNSUPPORTED(CMD_LOADASSET, false)
#define EVE_CoCmd_loadPatch(phost, options) EVE_COCMD_UNSUPPORTED(CMD_LOADPATCH, false)
#define EVE_CoCmd_loadWav(phost, dst, options) EVE_COCMD_UNSUPPORTED(CMD_LOADWAV, false)
#define EVE_CoCmd_playWav(phost, options) EVE_COCMD_UNSUPPORTED(CMD_PLAYWAV, false)
...
ESD_FUNCTION(EVE_CoCmd_i2sStartup, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(freq, Type = uint32_t, Default = 44100) // FREQUENCY_HZ
/**
 * @brief Send CMD_I2SSTARTUP to initialize I2S audio
 *
 * @param phost Pointer to Hal context
 * @param freq I2S sample frequency, in Hz
 */
static inline void EVE_CoCmd_i2sStartup(EVE_HalContext *phost, uint32_t freq)
{
	EVE_MULTI_TARGET_CHECK(CMD_I2SSTARTUP, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_dd(phost, CMD_I2SSTARTUP, freq);
}

ESD_FUNCTION(EVE_CoCmd_touchRd, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(addr, Type = uint32_t, Default = 0) // MEMORY_ADDRESS
ESD_PARAMETER(num, Type = uint32_t, Default = 0)
/**
 * @brief Send CMD_TOUCHRD to read data from touch controller
 *
 * @param phost Pointer to Hal context
 * @param addr Destination address in main memory. Must be 16-bit aligned
 * @param num Size of data, in bytes. Must be even
 */
static inline void EVE_CoCmd_touchRd(EVE_HalContext *phost, uint32_t addr, uint32_t num)
{
	EVE_MULTI_TARGET_CHECK(CMD_TOUCHRD, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_ddd(phost, CMD_TOUCHRD, addr, num);
}

/**
 * @brief Send CMD_TOUCHWR to write data to touch controller
 *
 * @param phost Pointer to Hal context
 * @param num Size of data, in bytes. Must be even
 * Data must follow this command in the command buffer
 */
static inline void EVE_CoCmd_touchWr(EVE_HalContext *phost, uint32_t num)
{
	EVE_MULTI_TARGET_CHECK(CMD_TOUCHWR, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_dd(phost, CMD_TOUCHWR, num);
}

#else

#define EVE_CoCmd_i2sStartup(phost, freq) EVE_COCMD_UNSUPPORTED(CMD_I2SSTARTUP, false)
#define EVE_CoCmd_touchRd(phost, addr, num) EVE_COCMD_UNSUPPORTED(CMD_TOUCHRD, false)
#define EVE_CoCmd_touchWr(phost, num) EVE_COCMD_UNSUPPORTED(CMD_TOUCHWR, false)
...
ESD_FUNCTION(EVE_CoCmd_ddrStartup, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
/**
 * @brief Send CMD_DDRSTARTUP to start up DDR operations
 *
 * @param phost Pointer to Hal context
 */
static inline void EVE_CoCmd_ddrStartup(EVE_HalContext *phost)
{
	EVE_MULTI_TARGET_CHECK(CMD_DDRSTARTUP, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_d(phost, CMD_DDRSTARTUP);
}

ESD_FUNCTION(EVE_CoCmd_ddrShutdown, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
/**
 * @brief Send CMD_DDRSHUTDOWN to shutdown DDR operations
 *
 * @param phost Pointer to Hal context
 */
static inline void EVE_CoCmd_ddrShutdown(EVE_HalContext *phost)
{
	EVE_MULTI_TARGET_CHECK(CMD_DDRSHUTDOWN, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_d(phost, CMD_DDRSHUTDOWN);
}

ESD_FUNCTION(EVE_CoCmd_graphicsFinish, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
/**
 * @brief Send CMD_GRAPHICSFINISH to wait until the render engine is idle
 *
 * @param phost Pointer to Hal context
 */
static inline void EVE_CoCmd_graphicsFinish(EVE_HalContext *phost)
{
	EVE_MULTI_TARGET_CHECK(CMD_GRAPHICSFINISH, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_d(phost, CMD_GRAPHICSFINISH);
}

ESD_FUNCTION(EVE_CoCmd_softBoot, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(mode, Type = uint32_t, Default = BOOT_REGULAR)
/**
 * @brief Send CMD_SOFTBOOT to perform bootstrap procedure
 *
 * @param phost Pointer to Hal context
 * @param mode Boot mode: BOOT_REGULAR, BOOT_WARM, BOOT_RAM0, BOOT_SAFETY, BOOT_FLASH, or BOOT_SD
 */
static inline void EVE_CoCmd_softBoot(EVE_HalContext *phost, uint32_t mode)
{
	EVE_MULTI_TARGET_CHECK(CMD_SOFTBOOT, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_dd(phost, CMD_SOFTBOOT, mode);
}

ESD_FUNCTION(EVE_CoCmd_watchdog, Type = void, Category = _GroupHidden, Inline, Include = "Esd_Core.h")
ESD_PARAMETER(phost, Type = EVE_HalContext *, Default = Esd_GetHost, Hidden, Internal, Static) // PHOST
ESD_PARAMETER(init_val, Type = uint32_t, Default = 0)
/**
 * @brief Send CMD_WATCHDOG to set the watchdog timer reset interval
 *
 * @param phost Pointer to Hal context
 * @param init_val Watchdog timeout initialization value, in clocks. For example, 72000000 for 1 second on a 72 MHz system
 */
static inline void EVE_CoCmd_watchdog(EVE_HalContext *phost, uint32_t init_val)
{
	EVE_MULTI_TARGET_CHECK(CMD_WATCHDOG, EVE_CHIPID >= EVE_BT820);
	EVE_CoCmd_dd(phost, CMD_WATCHDOG, init_val);
}

#else

#define EVE_CoCmd_ddrStartup(phost) EVE_COCMD_UNSUPPORTED(CMD_DDRSTARTUP, false)
#define EVE_CoCmd_ddrShutdown(phost) EVE_COCMD_UNSUPPORTED(CMD_DDRSHUTDOWN, false)
#define EVE_CoCmd_graphicsFinish(phost) EVE_COCMD_UNSUPPORTED(CMD_GRAPHICSFINISH, false)
#define EVE_CoCmd_softBoot(phost, mode) EVE_COCMD_UNSUPPORTED(CMD_SOFTBOOT, false)
#define EVE_CoCmd_watchdog(phost, init_val) EVE_COCMD_UNSUPPORTED(CMD_WATCHDOG, false)
