...
typedef struct EVE_GpuDefs
{
	...
	uint32_t RegPClkPol;
	uint32_t RegTagX;
	uint32_t RegVolSound;
	uint32_t RegJ1Cold;
	uint32_t RegJ1Int;
	uint32_t RegIntFlags;
	uint32_t RegCmdRead;
	uint32_t RegCTouchTouch0XY;
	uint32_t RegCTouchTouch1XY;
	uint32_t RegCTouchTouch2XY;
	uint32_t RegCTouchTouch3XY;
	uint32_t RegTouchTagXY;
	uint32_t RegTouchTransformA;
	uint32_t RegCrc;
	uint32_t RegTrim;
	uint32_t RegEJpgReady;
	uint32_t RegEJpgDebug;
	uint32_t RegDatestamp;
	uint32_t RegCmdBWrite;
	uint32_t RegCmdBSpace;
	uint32_t RegESpimDummy;
	uint32_t RegFlashDtr;
	uint32_t RegTracker;
	uint32_t RegPlayControl;
	uint32_t RamDl;
	uint32_t RomFontTableAddress;
	uint32_t RamGSizeMax;
	uint32_t LowFreqBound;
	uint32_t BitmapAddrMask;
	uint32_t PaletteAddrMask;
	uint32_t ScissorSizeShift;
	uint32_t TagDlMask;
	uint32_t BitmapHandleMask;
	uint32_t BitmapLayoutHStrideMask;
} EVE_GpuDefs;
...
extern EVE_GpuDefs EVE_GpuDefs_BT82X;
#endif
...
#if defined(FT9XX_PLATFORM) || defined(FT4222_PLATFORM) || defined(RP2040_PLATFORM) || defined(ZEPHYR_PLATFORM)
	uint32_t DeviceIdx;
...
#if defined(FT9XX_PLATFORM) || defined(FT4222_PLATFORM) || defined(MPSSE_PLATFORM) || defined(RP2040_PLATFORM) || defined(ZEPHYR_PLATFORM)
...
typedef struct EVE_HalContext
{
	...
	int ReadTimeout; /* Read timeout in bytes */
	bool ReadTimeoutOccured;
	int TransferLength; /* Length of the current transfer, writes will be padded in endTransfer */
#endif
...
#if defined(EVE_SUPPORT_DDR)
	uint32_t RamGSize; /* The installed RAM_G size */
	uint32_t RamGAvailable; /* RAM_G size, minus optional default swapchain and system reserved space */
#endif
...
#if defined(FT4222_PLATFORM) && defined(EVE_SUPPORT_DDR)
	/* Read buffer including polling bytes */
	uint8_t SpiRdBuf[0xFFFF];
#endif
...
#if (EVE_SUPPORT_CHIPID >= EVE_BT820)
	uint8_t DlStateBase; /* Base index of the DL state cache stack, adjusted during recursive render targets */
#endif
#endif
#if (EVE_DL_OPTIMIZE)
	uint8_t DlPrimitive;
...
#if (EVE_SUPPORT_CHIPID >= EVE_BT820) && (EVE_CMD_CACHE_RENDER_TARGET)
	// EVE_CoCmd_renderTarget(uint32_t dst, uint16_t fmt, int16_t w, int16_t h)
...
/* End data transfer. When writing to BT82XX, any writes not aligned to 4 bytes will be padded here */																									   
...
{
	// TODO_BT820: Based on selected EXTENDED mode, that is, single/multi-touch?
	return (EVE_CHIPID == EVE_BT820) || (EVE_CHIPID & 0x01) == 0x01;
}
...
/* PCLK derived from OSC  SYSPLL / TXCLKDIV / 2 */
static inline bool EVE_Hal_supportLvds(EVE_HalContext *phost)
{
...
static inline bool EVE_Hal_supportRenderTarget(EVE_HalContext *phost)
{
...
static inline bool EVE_Hal_supportHsf(EVE_HalContext *phost)
{
	...
	/* No longer supported on BT820, use a transformed render target instead. */
	return EVE_CHIPID >= EVE_BT817 && EVE_CHIPID < EVE_BT820;
#else
	return false;
#endif
}
...
static inline bool EVE_Hal_supportDdr(EVE_HalContext *phost)
{
...
/* Send a 5byte host command, applicable to BT82X series. First byte is always 0xFF, except for ACTIVE where it's 0x00 */
EVE_HAL_EXPORT void EVE_Hal_hostCommand5(EVE_HalContext *phost, EVE_82X_SPI_COMMANDS_T cmd, uint8_t param0, uint8_t param1, uint8_t param2);
...
EVE_HAL_EXPORT void EVE_Host_setPllSp1(EVE_HalContext *phost, EVE_CMD_SYS_PLL_FREQ val);
EVE_HAL_EXPORT void EVE_Host_setClkDiv(EVE_HalContext *phost, EVE_CMD_SYS_CLK_DIV val);
EVE_HAL_EXPORT void EVE_Host_setBootCfg(EVE_HalContext *phost, EVE_CMD_BOOT_CFG val);
EVE_HAL_EXPORT void EVE_Host_bootCfgEn(EVE_HalContext *phost, EVE_CMD_BOOT_CFG_EN val);
EVE_HAL_EXPORT void EVE_Host_setDdrType(EVE_HalContext *phost, EVE_CMD_DDR_TYPE val);
EVE_HAL_EXPORT void EVE_Host_setGpReg(EVE_HalContext *phost, EVE_CMD_GPREG val);
#endif
