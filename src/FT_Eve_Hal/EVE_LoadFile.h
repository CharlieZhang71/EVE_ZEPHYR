...
#if defined(RP2040_PLATFORM) || defined(ZEPHYR_PLATFORM)
typedef enum
{
	SDHOST_OK = 0, /* OK */
...
static inline bool EVE_Util_loadImageFile(EVE_HalContext *phost, uint32_t address, const char *filename, uint32_t *format)
{
	return EVE_Util_loadImageFile_ex(phost, address, filename, format, OPT_NODL);
}
...
inline bool EVE_Util_loadImageFileW(EVE_HalContext *phost, uint32_t address, const wchar_t *filename, uint32_t *format)
{
	return EVE_Util_loadImageFileW_ex(phost, address, filename, format, OPT_NODL);
}
...
/** Load a file into RAM_G (Unicode).
If transfered is set, the file may be streamed partially,
and will be kept open until EVE_Util_closeFile is called.
Filename may be omitted in subsequent calls */
