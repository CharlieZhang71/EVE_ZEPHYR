...
#define ESD_BITMAPHANDLE_RENDERED (255)
#endif
...
#if (EVE_SUPPORT_CHIPID >= EVE_BT820)
	uint8_t Pending[ESD_BITMAPHANDLE_CAP]; /* Pending DL swap and render */

	/* Track the values that were actually committed to hardware, these are the render target starting points */
	bool HwResized[ESD_BITMAPHANDLE_CAP];
	uint8_t HwPage[ESD_BITMAPHANDLE_CAP];

	/* Track page value first used by this frame */
	uint8_t InitialPage[ESD_BITMAPHANDLE_CAP];

	/* Track state changes */
	uint8_t Flags[ESD_BITMAPHANDLE_CAP];
#endif

} Esd_HandleState;

#define ESD_BHFLAGS_SIZE_SET (1 << 1)
#define ESD_BHFLAGS_EXPECTS_DEFAULT_SIZE (1 << 2)
#define ESD_BHFLAGS_PAGE_SET (1 << 3)
#define ESD_BHFLAGS_EXPECTS_INITIAL_PAGE (1 << 4)
...
/// Flag pending bitmap info writes for this display list as rendered. Restore expected initial state before restoring a saved display list based on flags
ESD_CORE_EXPORT void Esd_BitmapHandle_OnSwap(uint8_t *handleFlags, uint8_t *handleInitialPage);

/// Metrics for bitmap handle usage, excludes the scratch handle
