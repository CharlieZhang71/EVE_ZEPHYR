...
ESD_CORE_EXPORT void Esd_Render_MultiGradient(int16_t x, int16_t y, int16_t width, int16_t height, esd_argb32_t topLeft, esd_argb32_t topRight, esd_argb32_t bottomLeft, esd_argb32_t bottomRight)
{
...
#endif
	{
		...
		static uint8_t paletted_raw[8];
		paletted_raw[0] = (uint8_t)(color.values[0]& 0xFF);
		paletted_raw[1] = (uint8_t)(color.values[0] >> 8);
		paletted_raw[2] = (uint8_t)(color.values[1]& 0xFF);
		paletted_raw[3] = (uint8_t)(color.values[1] >> 8);
		paletted_raw[4] = (uint8_t)(color.values[2]& 0xFF);
		paletted_raw[5] = (uint8_t)(color.values[2] >> 8);
		paletted_raw[6] = (uint8_t)(color.values[3]& 0xFF);
		paletted_raw[7] = (uint8_t)(color.values[3] >> 8);
		EVE_Hal_wrMem(phost, addr, paletted_raw, 8);
