...
typedef struct Esd_BitmapInfo
{
	...
	// (Deprecated, use Type ESD_RESOURCE_DIRECTFLASH instead) Load from flash. Struscture has flash addresses et, rather than file names, if there is no flash filesystem
	uint32_t Flash : 1;

	// (Deprecated, use Type ESD_RESOURCE_FLASH instead) Prefer loading the bitmap into RAM_G, even if it's an ASTC stored on Flash
...
#define ESD_IS_FORMAT_PALETTED(format) ((format >= PALETTED565 && format <= PALETTED8) || (format == PALETTEDARGB8))
#else
#define ESD_IS_FORMAT_PALETTED(format) ((format >= PALETTED565 && format <= PALETTED8) || format == PALETTED)
#endif
...
ESD_IDENTIFIER(RGB6)
ESD_IDENTIFIER(RGB8)
ESD_IDENTIFIER(ARGB6)
ESD_IDENTIFIER(ARGB8)
ESD_IDENTIFIER(PALETTEDARGB8)
#endif
...
/// Update BitmapInfo metrics and format from CMD_GETIMAGE. Frees or truncates allocated memory if size mismatches
