...
typedef struct
{
	...
	uint16_t SpinnerX;
	uint16_t SpinnerY;
	uint16_t SpinnerStyle;
	uint16_t SpinnerScale;
	...
	bool CmdIsSpinner; //< Whether the current running function is the spinner
	...
	// int32_t TouchTransform[6];
	int32_t TouchTransformC; //< Touch transform matrix in s15.16
	int32_t TouchTransformF;
	bool TouchTransformValid;
#endif
...
#define ESD_FLASH_BT820 (EVE5 - EVE3)
#define ESD_FLASH_NB 3
#else
