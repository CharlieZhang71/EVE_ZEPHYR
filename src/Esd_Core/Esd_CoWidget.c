...
bool Esd_CoWidget_PlayBgVideo(Esd_BitmapCell video)
{
	...
	if (ESD_RESOURCE_IS_EVE_SDCARD(info->Type))
	{
		...
		uint32_t fsSourceRes = EVE_CoCmd_fsSource(phost, info->File);
		bool fsSourceSucceeds = Esd_CheckFsSourceRes(fsSourceRes, info->File);
		if (!fsSourceSucceeds)
			return false;
	}
#endif
...
#if (EVE_SUPPORT_CHIPID >= EVE_BT820)
	else if (ESD_RESOURCE_IS_EVE_SDCARD(info->Type))
	{
...
#if (EVE_SUPPORT_CHIPID >= EVE_BT820)
bool Esd_CoWidget_PlayVideoEveSdCard(const char *filename, uint16_t options)
{
#ifdef EVE_SUPPORT_VIDEO
	Esd_Context *ec = Esd_CurrentContext;
	EVE_HalContext *phost = Esd_GetHost();
	...
	EVE_MediaFifo_set(phost, fifoAddr, fifoSize);
	EVE_CoCmd_playVideo(phost, (options | OPT_MEDIAFIFO | OPT_FS) & ~(OPT_FLASH));
...
bool Esd_CoWidget_PlayVideo(Esd_BitmapCell video, uint16_t options)
{
	...
	{
		return Esd_CoWidget_PlayVideoEveSdCard(info->File, options);
	}
#endif
	else
	{
