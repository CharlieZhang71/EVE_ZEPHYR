...
ESD_CORE_EXPORT void Esd_TouchTag_Update(Esd_TouchTag *context)
{
	...
	if (context->Tap == (void *)Esd_RenderTarget_TouchTag)
	{
		// Special case, this redirects to tags within the render target
		if (s_TagDown == context->Tag || s_GpuRegTouchTag == context->Tag)
		{
			// Replace down and gpu reported tag
			// Up is not replaced, since it must equal whatever the last state was
			eve_tag_t rtTag = Esd_RenderTarget_TouchTag((void *)context->Down); // BitmapInfo is stored in Down
			// Sometimes returns 0 due to touch timing mismatch. Just leave it at the render target's tag if this happens
			// NOTE: If there's any flickering while holding a button, then there's an issue with touch tag timing
			// and we'd need to figure out a way to get the entire tag chain validated and reused, rather than just replacing here
			if (rtTag)
			{
				if (s_TagDown == context->Tag)
					s_TagDown = rtTag;
				if (s_GpuRegTouchTag == context->Tag)
					s_GpuRegTouchTag = rtTag;
			}
		}
		return;
	}
...
ESD_CORE_EXPORT uint8_t Esd_TouchTag_Tag(Esd_TouchTag *context)
{
	if (context->Tag)
		...
		return EVE_TAG_NONE; // Always return non-tag value if no tag
}
