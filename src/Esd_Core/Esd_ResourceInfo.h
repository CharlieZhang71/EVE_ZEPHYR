...
// (3-bit enum, see Esd_ResourceInfo::Type)
ESD_ENUM(Esd_ResourceType, DisplayName = "Resource Type")
// Load from host SD card or other filesystem
...
#define ESD_RESOURCE_DIRECTFLASH 3 /* This shares bit 2 with flash */
// Load from GPU SD card (handled by BT82X)
#define ESD_RESOURCE_SD 4
ESD_END()
...
// Resource is reloacatable
#define ESD_RESOURCE_ANIM_RELOC 3
// Load to RAM using CMD_VIDEOFRAME
// TBD: #define ESD_RESOURCE_VIDEOFRAME 4
ESD_END()
...
#define ESD_RESOURCE_IS_EVE_SDCARD(resourceType) (resourceType == ESD_RESOURCE_SD)
...
typedef struct Esd_ResourceInfo // (16 bytes) (24 bytes on 64 bit)
{
	...
	// Required only for resource types ESD_RESOURCE_PROGMEM and ESD_RESOURCE_FLASH
...
/* Available bitmap handles*/

/* One bitmap handle is reserved by the coprocessor, so it is not counted in allocatin metrics */
#define ESD_BITMAPHANDLE_RESERVED (1)

/* Runtime check, all bitmap handles that can technically be modified by the application */
#define ESD_BITMAPHANDLE_NB (EVE_CHIPID >= EVE_BT820 ? 64UL : (EVE_CHIPID >= EVE_FT810 ? 32UL : 16UL))

/* Hardcoded limit */
#if (EVE_SUPPORT_CHIPID >= EVE_BT820)
#define ESD_BITMAPHANDLE_CAP (64UL)
...
#define ESD_BITMAPHANDLE_INVALID 0xFF
