...
ESD_CORE_EXPORT Esd_GpuHandle Esd_GpuAlloc_Alloc_ex(Esd_GpuAlloc *ga, uint32_t size, uint16_t flags, uint16_t aligned)
{
	uint32_t idx;
	Esd_GpuHandle ret;
		    ...
		    && ga->AllocEntries[idx].Length >= size)
		{
			...
			for (id = 0; id < MAX_NUM_ALLOCATIONS; ++id)
			{
				if (ga->AllocRefs[id].Idx == MAX_NUM_ALLOCATIONS)
				{
					...
					if (aligned > 0)
					{
						ga->AllocEntries[idx].Address = (ga->AllocEntries[idx].Address + aligned - 1) & ~(aligned - 1);
					}
...
{
	return Esd_GpuAlloc_Alloc_ex(ga, size, flags, 0);
}
