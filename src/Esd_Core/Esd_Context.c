...
ESD_CORE_EXPORT void Esd_Defaults(Esd_Parameters *ep)
{
	...
	wcscpy_s(ep->FlashFilePaths[ESD_FLASH_BT820], _countof(ep->FlashFilePaths[ESD_FLASH_BT820]), L"__Flash.bin");
#endif
#else
...
#endif
#if (EVE_SUPPORT_CHIPID >= EVE_BT820)
	strcpy(ep->FlashFilePaths[ESD_FLASH_BT820], "__Flash.bin");
#endif
#endif
#endif
#ifdef ESD_MEMORYPOOL_ALLOCATOR
...
ESD_CORE_EXPORT bool Esd_Open(Esd_Context *ec, Esd_Parameters *ep)
{
...
#endif
	{
		BT8XXEMU_touchTransformReset(phost->Emulator);
#ifdef EVE_SUPPORT_RENDERTARGET
		// EVE_Hal_rdMem(phost, REG_TOUCH_TRANSFORM_A, (uint8_t *)&ec->TouchTransform, sizeof(ec->TouchTransform));
		ec->TouchTransformC = EVE_Hal_rd32(phost, REG_TOUCH_TRANSFORM_C);
		ec->TouchTransformF = EVE_Hal_rd32(phost, REG_TOUCH_TRANSFORM_F);
	...
	esd_scope()
	{
		...
		EVE_CoDl_clearTag(phost, EVE_TAG_NONE); // Always default to 255, so no touch = 0, touch non-tag = 255
	...
	ec->GpuAlloc.RamGSize = RAM_G_AVAILABLE;
...
#if (EVE_SUPPORT_CHIPID >= EVE_BT820)
	Esd_AttachEveSdCard(&ec->HalContext);
#endif
#if defined(EVE_FLASH_AVAILABLE)
	Esd_AttachFlashFast();
#endif
...
ESD_CORE_EXPORT void Esd_Render(Esd_Context *ec)
{
...
#if EVE_SUPPORT_CHIPID >= EVE_BT820
	if (EVE_CHIPID >= EVE_BT820 && ec->SpinnerPopped)
	{
		// On BT820 the spinner must be disabled once we start writing to the display list
	...
	if (ec->SpinnerPopup)
	{
		...
		EVE_CoDl_colorRgb_ex(phost, ~(ec->ClearColor));
		Esd_CoWidget_Spinner(Esd_Update, ec->SpinnerX, ec->SpinnerY, ec->SpinnerStyle, ec->SpinnerScale);
		ec->SpinnerPopup = false;
		ec->SpinnerPopped = true;
	}
	...
	if (!ec->SpinnerPopped)
	{
		...
		Esd_BitmapHandle_OnSwap(NULL, NULL); /* Call after swap, since this doesn't write any DL */
	}
