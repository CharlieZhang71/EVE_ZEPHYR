...
/* Pops up the spinner at the end of the frame until the next frame starts or is ready.
Same as `Esd_CoWidget_PopupSpinner` with all the parameters of `Esd_CoWidget_Spinner` */
ESD_FUNCTION(Esd_CoWidget_PopupSpinnerEx, DisplayName = "Pop-up Spinner Ex", Type = void, Attributes = ESD_CORE_EXPORT, Category = EsdUtilities, Include = "Esd_Core.h")
ESD_PARAMETER(x, Type = int16_t, Default = 0) // SCREEN_SIZE
ESD_PARAMETER(y, Type = int16_t, Default = 0) // SCREEN_SIZE
ESD_PARAMETER(style, Type = uint16_t, Default = 0) // TODO: Spinner style enum
ESD_PARAMETER(scale, Type = uint16_t, Default = 0)
ESD_CORE_EXPORT void Esd_CoWidget_PopupSpinnerEx(int16_t x, int16_t y, uint16_t style, uint16_t scale);
