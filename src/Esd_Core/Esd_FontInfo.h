...
typedef struct Esd_FontInfo // (40 bytes) (56 bytes on 64 bit)
{
	...
	// The font map for pre-BT82x chips, or the font resource for BT82x chips
	Esd_ResourceInfo FontResource;

	// BT82x use raw/.reloc file. glyph and x font is no longer used
...
{
	// (Runtime) Bitmap handle that is being used
	uint8_t BitmapHandle : 6; // NOTE: On BT820 this no longer fits the largest bitmap handle
