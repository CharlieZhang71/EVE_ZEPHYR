...
ESD_CORE_EXPORT uint32_t Esd_LoadRelocAnimation(Esd_AnimationInfo * animationInfo)
{
	...
	uint32_t objectAddress = Esd_LoadResource(&animationInfo->Object, NULL);
	// printf("Esd_LoadAnimation objectAddress %d\n", objectAddress);
	if (objectAddress == GA_INVALID)
	{
		esd_animationinfo_printf("Animation object resource cannot be loaded\n");
		return GA_INVALID;
	}
	// If this object was already validated before, just return as-is
	if (animationInfo->NumFrames > 0)
		return objectAddress;
	uint32_t signature = EVE_Hal_rd32(phost, objectAddress);
	uint32_t numFrames = EVE_Hal_rd32(phost, objectAddress + 4);
...
#else// Load animation for BT82X
	return Esd_LoadRelocAnimation(animationInfo);
#endif
}
