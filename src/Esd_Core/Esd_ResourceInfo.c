...
ESD_CORE_EXPORT uint32_t Esd_LoadResourceEx(Esd_ResourceInfo *resourceInfo, uint8_t *metadata, uint32_t *imageFormat)
{
	...
	if (metadata && (resourceInfo->Type == ESD_RESOURCE_FILE || resourceInfo->Type == ESD_RESOURCE_SD))
	{
	...
	uint16_t aligned = 0;
	if (resourceInfo->Compressed == ESD_RESOURCE_ANIM_RELOC)
	{
		aligned = 64; // 64-byte aligned for relocatable animation asset
	}
	else if (ESD_RESOURCE_IS_EVE_SDCARD(resourceInfo->Type) && resourceInfo->Compressed == ESD_RESOURCE_RAW)
	{
		aligned = 32; // 32-byte aligned for raw SD card resources
	}
	resourceInfo->GpuHandle = Esd_GpuAlloc_Alloc_ex(Esd_GAlloc, resourceInfo->RawSize,
	    (resourceInfo->Persistent ? 0 : GA_GC_FLAG)
	        | ((!resourceInfo->Compressed && ESD_RESOURCE_IS_FLASH(resourceInfo->Type)) ? GA_LOW_FLAG : 0), aligned);
#else
	...
	case ESD_RESOURCE_FILE: {
		...
		case ESD_RESOURCE_IMAGE:
			...
			break;
		case ESD_RESOURCE_ANIM_RELOC: 
			{
				uint32_t transfered = 0;
				EVE_CoCmd_loadAsset(phost, addr, 0);
				loaded = EVE_Util_loadCmdFile(phost, resourceInfo->File, &transfered);
				break;
			}
		}
		break;
	...
	case ESD_RESOURCE_DIRECTFLASH: {
		switch (resourceInfo->Compressed)
		{
			...
			case ESD_RESOURCE_IMAGE:
				...
				break;
			case ESD_RESOURCE_ANIM_RELOC: {
			    EVE_CoCmd_flashSource(phost, flashAddr);
			    EVE_CoCmd_loadAsset(phost, addr, OPT_FLASH);
			    loaded = true;
				break;
			}
		}
	...
	case ESD_RESOURCE_SD: {
		switch (resourceInfo->Compressed)
		{
			case ESD_RESOURCE_RAW:
			{
				uint32_t fsReadRes = EVE_CoCmd_fsRead(Esd_Host, addr, resourceInfo->File);
				loaded = Esd_CheckFsSourceRes(fsReadRes, resourceInfo->File);
				break;
			}
			case ESD_RESOURCE_DEFLATE:
			{
				uint32_t fsSourceRes = EVE_CoCmd_fsSource(phost, resourceInfo->File);
				bool fsSourceOK = Esd_CheckFsSourceRes(fsSourceRes, resourceInfo->File);
				loaded = fsSourceOK ? EVE_CoCmd_inflate_sdCard(Esd_Host, addr, resourceInfo->File) : false;
				break;
			}
			case ESD_RESOURCE_IMAGE:
			{
				uint32_t fsSourceRes = EVE_CoCmd_fsSource(phost, resourceInfo->File);
				bool fsSourceOK = Esd_CheckFsSourceRes(fsSourceRes, resourceInfo->File);
				loaded = fsSourceOK ? EVE_CoCmd_loadImage_sdCard(Esd_Host, addr, resourceInfo->File, imageFormat) : false;
				break;
			}
		    case ESD_RESOURCE_ANIM_RELOC: 
			{
			    ...
			    if (fsSourceRes != 0)
			    {
				    esd_resourceinfo_printf("SD read failed\n");
				    return;
			    }

			    EVE_CoCmd_loadAsset(phost, addr, OPT_FS);
			    loaded = true;
			    break;
		    }
		}
