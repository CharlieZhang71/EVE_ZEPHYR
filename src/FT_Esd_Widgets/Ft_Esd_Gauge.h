/*
This file is automatically generated
DO NOT MODIFY BY HAND
Ft_Esd_Gauge
Header
*/

#ifndef Ft_Esd_Gauge__H
#define Ft_Esd_Gauge__H

#include "EVE_Hal.h"
#include "Esd_Base.h"
#include "Ft_Esd.h"
#include "Ft_Esd_Math.h"
#include "Ft_Esd_PosSizeToRadius.h"
#include "Ft_Esd_Theme.h"
#include "Ft_Esd_Widget.h"

#ifndef ESD_LOGIC
#define ESD_LOGIC(name, ...)
#define ESD_ACTOR(name, ...)
#define ESD_WIDGET(name, ...)
#define ESD_PAGE(name, ...)
#define ESD_APPLICATION(name, ...)
#define ESD_INPUT(name, ...)
#define ESD_ARRAY_INPUT(name, ...)
#define ESD_OUTPUT(name, ...)
#define ESD_SLOT(name, ...)
#define ESD_SIGNAL(name, ...)
#define ESD_VARIABLE(name, ...)
#define ESD_WRITER(name, ...)
#endif

#define Ft_Esd_Gauge_CLASSID 0x13959398
ESD_SYMBOL(Ft_Esd_Gauge_CLASSID, Type = esd_classid_t)

/* EVE builtin Gauge Widget */
ESD_WIDGET(Ft_Esd_Gauge, Include = "Ft_Esd_Gauge.h", Callback, DisplayName = "ESD Gauge", Category = EsdWidgets, Icon = ":/icons/clock.png", X = 0, Y = 0, Width = 144, Height = 144)
typedef struct
{
	union
	{
		void *Owner;
		Ft_Esd_Widget Widget;
	};
	ESD_INPUT(Theme, Type = Ft_Esd_Theme *, Default = Ft_Esd_Theme_GetCurrent)
	Ft_Esd_Theme *(* Theme)(void *context);
	Ft_Esd_PosSizeToRadius Ft_Esd_PosSizeToRadius;
	ESD_INPUT(Alpha, Type = ft_uint8_t, Min = 0, Max = 255, SingleStep = 1, Default = 255)
	ft_uint8_t(* Alpha)(void *context);
	ESD_INPUT(Val, DisplayName = "Value", Type = ft_uint16_t, Max = 65535)
	ft_uint16_t(* Val)(void *context);
	ESD_INPUT(Range, DisplayName = "Range", Type = ft_uint16_t, Max = 65535)
	ft_uint16_t(* Range)(void *context);
	ESD_INPUT(Major, Type = ft_uint16_t, Min = 1, Max = 10, Default = 4)
	ft_uint16_t(* Major)(void *context);
	ESD_INPUT(Minor, Type = ft_uint16_t, Min = 1, Max = 10, Default = 16)
	ft_uint16_t(* Minor)(void *context);
	ESD_VARIABLE(AutoResize, Type = Esd_AutoResize, Default = ESD_AUTORESIZE_NONE, Public)
	ft_uint8_t AutoResize;
	ESD_INPUT(ShowTicks, DisplayName = "Show Ticks", Type = ft_bool_t, Default = true)
	ft_bool_t(* ShowTicks)(void *context);
	ESD_INPUT(ShowPointer, DisplayName = "Show Pointer", Type = ft_bool_t, Default = true)
	ft_bool_t(* ShowPointer)(void *context);
	ESD_INPUT(ShowBackground, DisplayName = "Show Background", Type = ft_bool_t, Default = true)
	ft_bool_t(* ShowBackground)(void *context);
} Ft_Esd_Gauge;

void Ft_Esd_Gauge__Initializer(Ft_Esd_Gauge *context);

ESD_SLOT(Render)
void Ft_Esd_Gauge_Render(Ft_Esd_Gauge *context);

ESD_SLOT(Update)
void Ft_Esd_Gauge_Update(Ft_Esd_Gauge *context);

#endif /* Ft_Esd_Gauge__H */

/* end of file */
