...
typedef struct Ft_Esd_Widget
{
	...
	union
	{
		struct
		{
			bool Active : 1; // Widget is actively being displayed, call Ft_Esd_Widget_SetActive to change this
			bool Recalculate : 1; // Widget layout needs to be recalculated. Set to 1 when user changes LocalRect on child object. Set to 1 when parent changes GlobalRect. Always recalculates all child GlobalRect variables
			bool Root : 1; // Widget with this flag set is controlled manually and cannot be added into any layout
			bool Instanced : 1; // Widget is allocated dynamically in memory and needs to be deleted
			bool GlobalValid : 1; // Global rectangle has been set by the parent layout
			bool DefaultIdle : 1; // This widget and none of the child widget implement an idle function
		};
		uint32_t Flags;
	};
...
void Ft_Esd_Widget_CheckDefaultIdle(Ft_Esd_Widget *context);
...
ESD_ENUM(Esd_CGradientType, Type = ft_uint8_t)
#define ESD_CORNER_ZERO (0)
#define ESD_EDGE_ZERO (1)
ESD_END()

ESD_ENUM(Esd_LayoutDirection, Type = ft_uint8_t)
#define ESD_HORIZONTAL (0)
#define ESD_VERTICAL (1)
ESD_END()

ESD_ENUM(Esd_DisplayStyle, Type = ft_uint8_t)
#define ESD_OPT_3D (0)
#define ESD_OPT_FLAT (1)
ESD_END()

ESD_ENUM(Esd_Spinner_Style, Type = ft_uint8_t)
#define ESD_SPINNER_CYCLE_DOT (0)
#define ESD_SPINNER_LINE_DOT (1)
#define ESD_SPINNER_ROTATE_CLOCK (2)
#define ESD_SPINNER_ORBIT_DOT (3)
ESD_END()

ESD_ENUM(Esd_Spinner_Scale, Type = ft_uint8_t)
#define ESD_SPINNER_SCALE_1X (0)
#define ESD_SPINNER_SCALE_2X (1)
#define ESD_SPINNER_SCALE_4X (2)
#define ESD_SPINNER_SCALE_8X (3)
ESD_END()

ESD_ENUM(Esd_AnimationLoop, Type = int8_t)
ESD_IDENTIFIER(ANIM_ONCE)
ESD_IDENTIFIER(ANIM_LOOP)
ESD_IDENTIFIER(ANIM_HOLD)
ESD_END()
