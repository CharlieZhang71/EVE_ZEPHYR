...
void Ft_Esd_Widget_FlagCustomIdle(Ft_Esd_Widget *context)
{
	context->DefaultIdle = false;
	Ft_Esd_Widget *parent = context->Parent;
	while (parent && parent->DefaultIdle)
	{
		parent->DefaultIdle = false;
...
{
	if (context->Slots->Idle != Ft_Esd_Widget_Idle)
	{
		Ft_Esd_Widget_FlagCustomIdle(context);
		return;
	}
	Ft_Esd_Widget *child = context->First;
	while (child)
	{
		if (!child->DefaultIdle)
		{
			Ft_Esd_Widget_FlagCustomIdle(context);
			return;
		}
...
{
	bool recalculate = context->Recalculate;
	if (recalculate)
	{
	...
	if (recalculate)
		Ft_Esd_Widget_CheckDefaultIdle(context);
}
...
{
	if (context->DefaultIdle)
		return; /* All child widgets are using the default idle funcion, skip */
